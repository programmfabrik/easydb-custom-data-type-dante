...
PATH_IN_CONTAINER=/config/plugin/custom-data-type-dante

CULTURES_CSV = l10n/cultures.csv
CULTURES_CSV_IN_CONTAINER = $(PATH_IN_CONTAINER)/l10n/cultures.csv

L10N_FILES = l10n/custom-data-type-dante.csv
L10N_FILES_IN_CONTAINER = $(PATH_IN_CONTAINER)/l10n/custom-data-type-dante.csv

JS_FILE = build/webfrontend/custom-data-type-dante.js
#COMMONS = src/library/commons.coffee

all: library ${JS_FILE} build-stamp-l10n
#all: ${COMMONS} ${JS_FILE} build-stamp-l10n
...
clean:
	...
	rm -f src/library/commons.coffee
	rm -f src/library/commons.coffee.js
...
${JS_FILE}: src/library/commons.coffee.js src/webfrontend/CustomDataTypeDante.coffee.js
