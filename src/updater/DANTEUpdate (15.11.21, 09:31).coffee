  ...
  __updateData: ({objects, plugin_config, state}) ->
    ...
    for DANTEUri, key in DANTEUris
      do(key, DANTEUri) ->
        ...
        #console.error "444", DANTEUri
        extendedInfo_xhr = new (CUI.XHR)(url: DANTEUri)
        xhrDeferredDANTERequest = extendedInfo_xhr.start()
        .done((data, status, statusText) ->
          console.error "XHR for " + DANTEUri + " done --> resolve!"
          ...
          if data?.length == 1
            ...
            if data?.prefLabel
              ...
              for cdataFromObjectsMap, objectsMapKey in objectsMap[originalDANTEUri]
                ...
                # _standard & _fulltext
                # TODO TODO IN DER FUNKTION WIRFT ES EINEN FEHLER!!!
                # TODO TODO IN DER FUNKTION WIRFT ES EINEN FEHLER!!!
                # TODO TODO IN DER FUNKTION WIRFT ES EINEN FEHLER!!!
                ...
                updatedDANTEcdata.conceptName = updatedDANTEcdata.conceptName + '_fromUpdater'
        ...
        xhrPromises.push xhrDeferredDANTERequest

    CUI.whenAll(xhrPromises).done( =>
      console.error "+++++++++ EVERYTHING DONE!!!"
      #console.error "All Requests (" + objectsToUpdate.length + ") from package done!"
      ez5.respondSuccess({payload: []})
